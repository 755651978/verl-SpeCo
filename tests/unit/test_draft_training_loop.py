...
def test_standalone_dspark_checkpoint_rewrites_generic_qwen3_architecture(tmp_path):
    ...
    (target_dir / "config.json").write_text(json.dumps({"model_type": "qwen3"}), encoding="utf-8")
    (checkpoint_dir / "config.json").write_text(
        json.dumps(
            {
                "model_type": "dspark",
...
def test_standalone_block_checkpoint_uses_target_model_type_without_source_config(tmp_path):
    checkpoint_dir = tmp_path / "draft_step_5"
    checkpoint_dir.mkdir()
    target_dir = tmp_path / "target_qwen3"
    target_dir.mkdir()
    ...
    trainer = SimpleNamespace(
        ...
        config=SimpleNamespace(
            model=SimpleNamespace(path=str(target_dir)),
            rollout=SimpleNamespace(drafter=SimpleNamespace(model_path=str(missing_source_dir))),
        ),
    )
    ...
    assert runtime_config["dspark_config"]["markov_head_type"] == "vanilla"
    assert runtime_config["rope_parameters"] == {"rope_theta": 1000000.0, "rope_type": "default"}
...
def test_standalone_dflash_checkpoint_preserves_source_lm_head(tmp_path):
    ...
    safetensors_torch.save_file({"lm_head.weight": torch.ones(3, 4)}, str(source_dir / "model.safetensors"))
...
def test_standalone_dflash_checkpoint_does_not_create_lm_head_without_source(tmp_path):
    ...
    safetensors_torch.save_file({"model.embed_tokens.weight": torch.ones(3, 4)}, str(target_dir / "model.safetensors"))
    safetensors_torch.save_file({"fc.weight": torch.ones(2, 2)}, str(checkpoint_dir / "model.safetensors"))
    trainer = SimpleNamespace(
        backend=SimpleNamespace(model_type="dflash"),
        config=SimpleNamespace(
...
def test_standalone_dspark_checkpoint_appends_target_tied_embedding(tmp_path):
    ...
    safetensors_torch.save_file({"model.embed_tokens.weight": embedding}, str(target_dir / "model.safetensors"))
    safetensors_torch.save_file({"fc.weight": torch.ones(2, 2)}, str(checkpoint_dir / "model.safetensors"))
    trainer = SimpleNamespace(
        backend=SimpleNamespace(model_type="dspark"),
        config=SimpleNamespace(
