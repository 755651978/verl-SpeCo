...
class _DrafterOptimizerState:
    ...
    def state_dict(self):
        ...
        return get_optimizer_state_dict(
            self.model, self.optimizer, options=self._options()
        )
...
def log_alignment_event(
    logger: logging.Logger, payload: dict[str, Any], level: int = logging.INFO
) -> None:
...
def _target_top_ids(target_logprobs: torch.Tensor, row: int, limit: int) -> list[int]:
    if (
        target_logprobs is None
        or target_logprobs.numel() == 0
        or row >= target_logprobs.size(0)
    ):
        return []
...
def _alignment_window_rows(
    ...
    *,
    ...
    if target_logprobs is None:
        return []
    target_rows = min(
        target_logprobs.size(0),
        max(input_ids.size(0) - 1, 0),
        max(loss_mask.size(0) - 1, 0),
    )
    if target_rows <= 0:
        return []
        ...
        rows.append(
            {
                ...
                "top_ids": top_ids,
                "contains_predict": predict_token in top_ids
...
class DrafterBaseTrainer:
    def __init__(
        ...
        if self.training_device_mesh is not None:
            ...
            self.training_group_world_size = (
                dist.get_world_size(training_process_group)
                if training_process_group is not None
                else 1
            )
            self.dp_group_world_size = (
                dist.get_world_size(data_parallel_process_group)
                if data_parallel_process_group is not None
                else 1
            )
            self.rank = (
                dist.get_rank(training_process_group)
                if training_process_group is not None
                else (dist.get_rank() if dist.is_initialized() else 0)
            )
            self.dp_rank = (
                dist.get_rank(data_parallel_process_group)
                if data_parallel_process_group is not None
                else 0
            )
        ...
        self.runtime_device = (
            torch.device(f"{device_name}:{self.device_id}")
            if device_name != "cpu"
        ...
        self.batch_size = int(
            self.config.rollout.drafter.training.get("batch_size_per_gpu", 4)
        )

        # Initialize DataBuffer for storing data across RL steps
        buffer_max_size = int(
            self.config.rollout.drafter.training.get("data_buffer_max_size", 10000)
        )
        # Only store hidden states in buffer if we're collecting them during generation
        collect_hidden_states_from_sgl = bool(
        ...
        self.data_buffer = DataBuffer(
            max_size=buffer_max_size, store_hidden_states=collect_hidden_states_from_sgl
        )
        ...
        self.enable_mesh_barrier = bool(
            self.config.rollout.drafter.training.get("enable_step_barrier", False)
        )
        ...
        if self._is_block_drafter_backend():
            ...
            if rollout_tp_size > 1 and self.training_group_world_size > 1:
                logger.debug(
                    ...
                    "rollout_tp=%s training_group_world_size=%s",
                    self.rank,
                    self.config.rollout.drafter.get(
                        "speculative_algorithm", self.backend.model_type
                    ),
                    rollout_tp_size,
                    self.training_group_world_size,
                )
        ...
        for state in self.optimizer.state.values():
            ...
            for value in state.values():
                if (
                    ...
                    and value.device.type != self.runtime_device.type
                ):
                    return False
        return True
    ...
    def _create_copy_stream(self):
        ...
        try:
            ...
            logger.debug(
                f"Failed to create drafter copy stream on {device_name}: {exc}"
            )
            return None
    ...
    def get_training_metrics(self) -> dict[str, float]:
        ...
        if quality_tokens > 0:
            metrics[f"{prefix}/top1_acc"] = (
                sums.get(f"{prefix}/top1_correct_count", 0.0) / quality_tokens
            )
        ...
        if ce_tokens > 0:
            metrics[f"{prefix}/ce_loss"] = (
                sums.get(f"{prefix}/ce_loss_sum", 0.0) / ce_tokens
            )
    ...
    def record_training_timing(self, metric_name: str, elapsed_sec: float) -> None:
        ...
        key = (
            ...
            else f"timing_s/{metric_name}"
        )
        self._training_metric_sums[key] = self._training_metric_sums.get(
            key, 0.0
        ) + float(elapsed_sec)
    ...
    def _build_draft_model(self):
        ...
        if (
            ...
            setattr(
                self.backend,
                "_initial_target_lm_head_shape",
                tuple(pending_target_weight.shape),
            )
        else:
        ...
        if self.fsdp_device_mesh is not None and dist.is_initialized():
            ...
            auto_wrap_policy = get_fsdp_wrap_policy(
                module=raw_model, config=fsdp_config.wrap_policy
            )
            ...
            if self.use_native_dp_sp:
                logger.debug(
                    "Building drafter model with native dp x sp hybrid-shard FSDP"
                )
            ...
            logger.debug(
                "Building drafter model without FSDP (standalone/local trainer mode)"
            )
            self.model = raw_model
        ...
        resume_metadata = {}
        resume_state = {}
        resume_setting = drafter_train_config.get(
            "resume_trainer_state_from_checkpoint", None
        )
        ...
        if resume_enabled:
            ...
            if (
                not resume_state
                and get_drafter_checkpoint_step(spec_model_path) is not None
            ):
                logger.warning(
        ...
        resume_optimizer_steps = max(
            int(resume_state.get("optimizer_steps_total", 0) or 0), 0
        )
        resume_training_steps = max(
            int(resume_state.get("training_steps", resume_optimizer_steps) or 0), 0
        )
        ...
        if resume_enabled and resume_metadata.get("optimizer") is not None:
            restored_trainer_state = self._load_optimizer_checkpoint(
                spec_model_path, resume_metadata
            )
            restored_optimizer_lrs = [
                float(param_group["lr"]) for param_group in self.optimizer.param_groups
            ]
        self.lr_scheduler = self.backend.setup_scheduler(
            self.optimizer, drafter_train_config
        )
        if restored_trainer_state is not None:
            ...
            if self.lr_scheduler is not None and isinstance(scheduler_state, dict):
                ...
                restored_optimizer_lrs = [
                    float(lr) for lr in self.lr_scheduler.get_last_lr()
                ]
        ...
        self.pad_token_id = int(
            getattr(drafter_model_config, "pad_token_id", self.pad_token_id)
    ...
    def _prepare_training_config(self, rollout_config):
        ...
        with open_dict(drafter_train_config):
            ...
            drafter_train_config.update(
                {
                    "speculative_algorithm": rollout_config["drafter"][
    ...
    def _infer_pretrained_save_kwargs(self) -> dict[str, Any]:
        ...
        for index_path in sorted(
            glob.glob(os.path.join(spec_model_path, "*.index.json"))
        ):
            try:
                ...
                logger.warning(
                    "Failed to inspect drafter weight index %s: %s", index_path, exc
                )
                continue
        ...
        for filename in ("generation_config.json",):
            ...
            if os.path.exists(src) and not os.path.exists(dst):
                try:
                    ...
                    logger.warning(
                        "Failed to copy drafter %s to full checkpoint: %s",
                        filename,
                        exc,
                    )
        ...
        for pattern in (
            ...
            for path in glob.glob(os.path.join(output_dir, pattern)):
                try:
                    ...
                    logger.warning(
                        "Failed to remove stale drafter checkpoint file %s: %s",
                        path,
                        exc,
                    )
    ...
    def _checkpoint_process_group(self):
        ...
        if group is None:
            raise RuntimeError(
                "Drafter checkpoint requires an explicit SP process group"
            )
        return group

    def _sync_checkpoint_phase_error(
        self, error: Optional[BaseException], phase: str
    ) -> None:
        group = self._checkpoint_process_group()
        if group is None:
            if error is not None:
                raise RuntimeError(
                    f"Drafter checkpoint {phase} failed: {error}"
                ) from error
            return
    ...
    def _save_optimizer_checkpoint(self, checkpoint_path: str) -> dict[str, Any]:
        ...
        if is_leader:
            try:
                ...
                logger.warning(
                    "Unable to measure drafter optimizer checkpoint size at %s: %s",
                    optimizer_path,
                    exc,
                )
        return manifest
    ...
    def _load_optimizer_checkpoint(
        ...
        if optimizer_path is None:
            raise RuntimeError(
                f"Drafter checkpoint metadata declares no optimizer state: {checkpoint_path}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to restore drafter optimizer checkpoint from {optimizer_path}: {exc}"
            ) from exc

        if (
            not isinstance(trainer_state, dict)
            or int(trainer_state.get("version", 0) or 0) != 1
        ):
    ...
    def _save_pretrained_checkpoint_async(
        ...
        if self._pending_full_checkpoint_future is not None:
            ...
            try:
                ...
                raise RuntimeError(
                    "Previous full drafter checkpoint save failed"
                ) from exc
        ...
        if not hasattr(export_model, "save_pretrained"):
            raise TypeError(
                f"Drafter export model does not support save_pretrained: {type(export_model)}"
            )
        ...
        trainer_state = {
            ...
            "lr_scheduler_last_epoch": (
                int(self.lr_scheduler.last_epoch)
                if self.lr_scheduler is not None
                else None
            ),
            "current_lr": (
        ...
        if self._full_checkpoint_executor is None:
            self._full_checkpoint_executor = ThreadPoolExecutor(
                max_workers=1, thread_name_prefix="drafter-full-ckpt"
            )
        future = self._full_checkpoint_executor.submit(_write_full_checkpoint)
        self._pending_full_checkpoint_future = future
        logger.debug(
            "[Rank %s] Scheduled drafter checkpoint export to %s",
            self.rank,
            checkpoint_path,
        )
        return future
    ...
    def _save_checkpoint_async(
        ...
        return self._save_pretrained_checkpoint_async(
            checkpoint_path,
            step,
            optimizer_manifest,
        )
    ...
    def save_checkpoint(
        ...
        is_fsdp_wrapped = (
            isinstance(self.model, FSDP) or self.training_device_mesh is not None
        )
        load_elapsed = 0.0
        ...
        try:
            ...
            if wait and future is not None:
                ...
                if isinstance(write_result, dict):
                    export_elapsed = float(
                        write_result.get("export_elapsed", 0.0) or 0.0
                    )
                    write_elapsed = float(write_result.get("write_elapsed", 0.0) or 0.0)
                    reclaim_elapsed = float(
                        write_result.get("reclaim_elapsed", 0.0) or 0.0
                    )
        finally:
            if is_fsdp_wrapped and not was_on_device:
    ...
    def sync_target_lm_head_weight(
        ...
        if row_indices is not None and torch.is_tensor(row_indices):
            self._pending_target_lm_head_row_indices = (
                row_indices.detach().cpu().long().reshape(-1).contiguous()
            )
        elif isinstance(row_indices, (list, tuple)):
            self._pending_target_lm_head_row_indices = (
                torch.tensor([int(idx) for idx in row_indices], dtype=torch.long)
                .reshape(-1)
                .contiguous()
            )
        else:
    ...
    def _build_target_lm_head_row_indices_from_dflash_data(
        ...
        row_indices = (
            torch.unique(torch.cat(token_chunks), sorted=True)
            .to(dtype=torch.long)
            .contiguous()
        )
    ...
    def _build_target_lm_head_row_indices_from_t2d(
        self, t2d: Optional[torch.Tensor]
    ) -> Optional[dict[str, Any]]:
        ...
        row_indices = (
            torch.nonzero(t2d_cpu, as_tuple=False)
            .flatten()
            .to(dtype=torch.long)
            .contiguous()
        )
    ...
    def _build_target_lm_head_row_indices_from_drafter_config(
        self,
    ) -> Optional[dict[str, Any]]:
    ...
    def _load_target_lm_head_row_indices_from_checkpoint(
        self,
    ) -> Optional[dict[str, Any]]:
        model_path = self.config.rollout.drafter.get("model_path")
        ...
        try:
            ...
            logger.debug(
                "Failed to load EAGLE3 t2d from drafter checkpoint %s: %s",
                model_path,
                exc,
            )
            return None
    ...
    def _load_checkpoint_tensor(
        self, model_path: str, logical_name: str
    ) -> torch.Tensor:
        ...
        if index_paths:
            ...
            selected_key = self._select_checkpoint_tensor_key(
                weight_map.keys(), logical_name
            )
            if selected_key is None:
                raise KeyError(
                    f"Cannot find {logical_name} in checkpoint index {index_paths[0]}"
                )
    ...
    def _load_tensor_from_checkpoint_file(
        self, ckpt_file: str, logical_name: str
    ) -> torch.Tensor:
        ...
        if not isinstance(state_dict, dict):
            raise TypeError(
                f"Unsupported checkpoint object type for {ckpt_file}: {type(state_dict)}"
            )
        selected_key = self._select_checkpoint_tensor_key(
            state_dict.keys(), logical_name
        )
        if selected_key is None:
            raise KeyError(f"Cannot find {logical_name} in {ckpt_file}")
    ...
    def _target_lm_head_module(self):
        ...
        if (
            ...
            and getattr(target_lm_head, "fc", None) is not None
        ):
    ...
    def _apply_pending_target_lm_head_weight(self) -> bool:
        ...
        if source_row_indices is not None:
            ...
            if source_vocab_size is not None and int(target_weight.shape[0]) != int(
                source_vocab_size
            ):
                lm_head = self._resize_target_lm_head_to_vocab_size(
                    lm_head, int(source_vocab_size)
                )
                target_weight = lm_head.weight
            ...
            if (
                ...
                or int(source_row_indices.min().item()) < 0
            ):
                raise ValueError(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            row0_norm = (
                target_weight[0].detach().float().norm()
                if int(target_weight.size(0)) > 0
                else None
            )
            row_last_norm = (
                target_weight[-1].detach().float().norm()
                if int(target_weight.size(0)) > 0
                else None
            )
            logger.debug(
                ...
                else round(float(row0_norm.detach().cpu().item()), 6),
                None
                if row_last_norm is None
                else round(float(row_last_norm.detach().cpu().item()), 6),
            )
        if last_hidden_logprob_check_enabled() and int(target_weight.size(0)) > 0:
            probe_indices = sorted(
                {
                    ...
                    min(
                        int(target_weight.size(0)) // 2, int(target_weight.size(0)) - 1
                    ),
            ...
            for row_idx in probe_indices:
                ...
                probe_norms[f"row_{row_idx}_norm"] = round(
                    float(row_norm.detach().cpu().item()), 6
                )
            logger.debug(
    ...
    def _resize_target_lm_head_to_vocab_size(self, lm_head, vocab_size: int):
        ...
        if (
            target_lm_head is not None
            and getattr(target_lm_head, "fc", None) is lm_head
        ):
    ...
    def _resize_target_lm_head_to_weight(self, lm_head, source_weight: torch.Tensor):
        if source_weight.dim() != 2 or int(source_weight.shape[1]) != int(
            lm_head.in_features
        ):
            return lm_head
    ...
    def collect_online_data(
        ...
        elif target_logprobs is not None and not isinstance(
            target_logprobs, torch.Tensor
        ):
            logger.debug(
                f"[Rank {self.rank}] Unsupported target_logprobs type: {type(target_logprobs)}"
            )
            target_logprobs = None
        ...
        hidden_raw_target_logprobs_positions = batch.get(
        ...
        use_copy_stream = self.copy_stream is not None and any(
            isinstance(t, torch.Tensor) and t.device.type == device_name
        ...
        if use_copy_stream:
            with self.device_module.stream(self.copy_stream):
                ...
                cpu_target_logprobs = (
                    target_logprobs.to("cpu", non_blocking=True)
                    if target_logprobs is not None
                    else None
                )
                cpu_hidden_positions = (
                    hidden_positions.to("cpu", non_blocking=True)
            ...
            cpu_target_logprobs = (
                target_logprobs.to("cpu") if target_logprobs is not None else None
            )
            cpu_hidden_positions = (
                hidden_positions.to("cpu") if hidden_positions is not None else None
            )
            cpu_hidden_raw_target_logprobs = (
                hidden_raw_target_logprobs.to("cpu")
                if hidden_raw_target_logprobs is not None
                else None
            )
            cpu_hidden_raw_target_logprobs_positions = (
        ...
        for i in range(batch_size):
            ...
            if (
                ...
                and cpu_hidden_raw_target_logprobs_positions is not None
            ):
                raw_rows_for_alignment = int(cpu_hidden_raw_target_logprobs[i].size(0))
                candidate_raw_positions = (
                    cpu_hidden_raw_target_logprobs_positions[i].reshape(-1).long()
                )
                if int(candidate_raw_positions.numel()) >= raw_rows_for_alignment:
                    raw_positions_item_for_alignment = candidate_raw_positions[
                        :raw_rows_for_alignment
                    ]
            hidden_positions_item = None
            if cpu_hidden_positions is not None:
                hidden_positions_item = (
                    cpu_hidden_positions[i].reshape(-1)[:hidden_seq_length].long()
                )
                if hidden_positions_item.numel() <= 0:
                    hidden_positions_item = None
            hidden_position_start = _batch_item_int(
            ...
            if hidden_positions_item is not None:
                ...
                contiguous_mask = (
                    hidden_positions_item[1:] == hidden_positions_item[:-1] + 1
                )
                if contiguous_mask.numel() > 0 and not bool(contiguous_mask.all()):
                    first_break = (
                        int(torch.nonzero(~contiguous_mask, as_tuple=False)[0].item())
                        + 1
                    )
                ...
                if cpu_hidden_raw_target_logprobs is not None:
                    ...
                    if raw_positions_item_for_alignment is None:
                        logger.warning(
                            "[Rank %s] Drop logits sample with missing raw positions",
                            self.rank,
                        )
                        continue
                    common_rows = min(
                    ...
                    raw_contiguous = (
                        ...
                        == raw_positions_item_for_alignment[: max(common_rows - 1, 0)]
                        + 1
                    )
                    if raw_contiguous.numel() > 0 and not bool(raw_contiguous.all()):
                        raw_break = (
                            int(
                                torch.nonzero(~raw_contiguous, as_tuple=False)[0].item()
                            )
                            + 1
                        )
                    ...
                    if aligned.numel() > 0 and not bool(aligned.all()):
                        first_mismatch = int(
                            torch.nonzero(~aligned, as_tuple=False)[0].item()
                        )
                        common_rows = min(common_rows, first_mismatch)
                    selected_hidden_row_end = min(selected_hidden_row_end, common_rows)
                elif use_logits:
                    logger.warning(
                        "[Rank %s] Drop logits sample without raw top-k rows", self.rank
                    )
                    continue

                if selected_hidden_row_end <= 0:
                    logger.warning(
                        "[Rank %s] Drop sample with no aligned continuous prefix",
                        self.rank,
                    )
                    continue
                ...
                if hidden_position_start is None:
                    hidden_position_start = max(
                        expected_hidden_rows - hidden_seq_length, 0
                    )
            ...
            if cpu_target_logprobs is not None:
                ...
                target_logprobs_position_start = _batch_item_int(
                    batch.get("target_logprobs_position_start"), i
                )
                target_logprobs_position_end = _batch_item_int(
                    batch.get("target_logprobs_position_end"), i
                )
            ...
            if cpu_prompts is not None and cpu_responses is not None:
                ...
                response_len = min(
                    cpu_responses[i].numel(), max(0, input_seq_length - prompt_len)
                )
                if response_len > 0:
                    response_mask = (cpu_responses[i, :response_len] != pad_id).float()
                    full_loss_mask[prompt_len : prompt_len + response_len] = (
                        response_mask
                    )
                ...
                full_loss_mask[response_start:] = (
                    cpu_responses[i, :response_len] != pad_id
                ).float()
            else:
                full_loss_mask[:] = 1.0
            ...
            if cpu_target_logprobs is not None:
                ...
                target_start = min(
                    max(feature_start + target_row_offset - target_base, 0),
                    target_limit,
                )
                target_end = min(
                    max(feature_end - 1 - target_base, target_start), target_limit
                )
                target_logprobs_item = cpu_target_logprobs[
                    i, target_start:target_end, ...
                ]

            kept_hidden_positions = (
                hidden_positions_item[hidden_start:hidden_end]
                if hidden_positions_item is not None
                else None
            )
            ...
            if cpu_hidden_raw_target_logprobs is not None:
                ...
                if cpu_hidden_raw_target_logprobs_positions is not None:
                    ...
                    if int(candidate_raw_positions.numel()) >= raw_rows:
                        raw_target_logprobs_positions_item = candidate_raw_positions[
                            :raw_rows
                        ]
                raw_position_start = _batch_item_int(
                    batch.get("hidden_raw_target_logprobs_position_start"), i
                )
                raw_position_end = _batch_item_int(
                    batch.get("hidden_raw_target_logprobs_position_end"), i
                )
                ...
                if raw_target_logprobs_item.dim() == 3 and hidden_feature_length > 0:
                    ...
                    if raw_target_logprobs_positions_item is not None:
                        raw_index_by_position = {
                            ...
                            for raw_index, position in enumerate(
                                raw_target_logprobs_positions_item.tolist()
                            )
                            if int(position) >= 0
                        }
                        local_rows = []
                        ...
                        if local_rows:
                            hidden_raw_target_logprobs_item[
                                torch.tensor(local_rows, dtype=torch.long)
                            ] = raw_target_logprobs_item[
                                torch.tensor(raw_indices, dtype=torch.long)
                            ]
                    else:
                        ...
                        raw_slice_end = min(
                            max(int(hidden_end), raw_slice_start), raw_rows
                        )
                        copy_rows = min(
                            raw_slice_end - raw_slice_start, hidden_feature_length
                        )
                        if copy_rows > 0:
                            hidden_raw_target_logprobs_item[:copy_rows] = (
                                raw_target_logprobs_item[
            ...
            if accept_len is None:
                num_correct = _batch_item_float(
                    batch.get("num_correct_drafts_per_req_cpu"), i
                )
            ...
            if accept_len is not None:
                verify_tokens = int(
                    self.config.rollout.drafter.rollout.get("spec_verify_tokens", 0)
                    or 0
                )
                if verify_tokens > 0:
            ...
            data_item = {
                ...
                "_verl_target_start": target_start
                if cpu_target_logprobs is not None
                else None,
                "_verl_target_end": target_end
                if cpu_target_logprobs is not None
                else None,
                "_verl_target_position_start": (
                    (target_logprobs_position_start or 0) + target_start
                    if target_start is not None
                    else None
                ),
                "_verl_target_position_end": (
                    (target_logprobs_position_start or 0) + target_end
                ...
                "_verl_response_len": response_len
                if cpu_responses is not None
                else None,
                ...
                "hidden_target_logprobs_source": batch.get(
                    "hidden_target_logprobs_source"
                ),
                "hidden_raw_topk_logprob_check": batch.get(
                ...
                if should_log_alignment(
                    ...
                    log_alignment_event(
                        logger,
                        {
                            ...
                            "target_position_start": data_item.get(
                                "_verl_target_position_start"
                            ),
                            "target_position_end": data_item.get(
                                "_verl_target_position_end"
                            ),
                            "target_tensor_position_start": target_logprobs_position_start,
                            "target_tensor_position_end": target_logprobs_position_end,
                            "hidden_lm_head_fingerprint": data_item.get(
                                "hidden_lm_head_fingerprint"
                            ),
                            "hidden_last_hidden_logprob_check": data_item.get(
                                "hidden_last_hidden_logprob_check"
                            ),
                            "hidden_target_logprobs_source": data_item.get(
                                "hidden_target_logprobs_source"
                            ),
                            "hidden_raw_topk_logprob_check": data_item.get(
                                "hidden_raw_topk_logprob_check"
                            ),
                            "hidden_raw_target_shape": _tensor_shape(
                                data_item.get("hidden_raw_target_logprobs")
                            ),
                            ...
                            "hidden_raw_target_position_end": data_item.get(
                                "_verl_hidden_raw_target_position_end"
                            ),
                            "hidden_last_hidden_filter": data_item.get(
                                "hidden_last_hidden_filter"
                            ),
                            "hidden_last_hidden_select": data_item.get(
                                "hidden_last_hidden_select"
                            ),
                            "item_global_step": data_item.get("global_step"),
                            "prompt_len": prompt_len
                            if cpu_prompts is not None
                            else None,
                            "response_len": response_len
                            if cpu_responses is not None
                            else None,
                            "loss_before": int(full_loss_mask.sum().item()),
                            "loss_after_slice": int(
                                data_item["loss_mask"].sum().item()
                            ),
                            "target_rows": int(target_logprobs_item.size(0))
                            if target_logprobs_item is not None
                            else None,
    ...
    def _sanitize_sequence_tensor(
        ...
        if tensor.dim() < 3:
            raise ValueError(
                f"{name} must have shape [batch, seq, ...], got {tuple(tensor.shape)}"
            )
        ...
        if clip_value is not None:
            row_abs_max = (
                safe_tensor.detach().float().abs().flatten(start_dim=2).amax(dim=-1)
            )
    ...
    def _mask_loss_for_bad_inputs(
        ...
        masked_tokens = int(
            ((loss_mask > 0) & bad_target_rows).detach().sum().cpu().item()
        )
        if masked_tokens > 0:
            logger.debug(
                f"[Rank {self.rank}] Masked {masked_tokens} drafter targets due to bad input hidden rows"
            )
    ...
    def _sanitize_training_batch(
        ...
        batch["loss_mask"] = torch.where(
            loss_mask > 0, torch.ones_like(loss_mask), torch.zeros_like(loss_mask)
        )
        ...
        for target_key in ("target", "last_hidden_states", "target_last_hidden_states"):
            if target_key not in batch:
                continue
            target_tensor, bad_target_rows = self._sanitize_sequence_tensor(
                batch[target_key], target_key, clip_value
            )
            batch[target_key] = target_tensor
            masked_tokens = int(
                ((batch["loss_mask"] > 0) & bad_target_rows).detach().sum().cpu().item()
            )
            if masked_tokens > 0:
                logger.debug(
    ...
    def _sample_training_items(
        ...
        hard_ratio = float(
            self._block_drafter_config_value("hard_sample_ratio", 0.0) or 0.0
        )
    ...
    def _prepare_training_batch(
        ...
        if self._is_block_drafter_backend() and self.use_ulysses_sp:
            raise NotImplementedError(
                f"{self.backend.model_type} drafter training does not support Ulysses sequence parallel yet"
            )

        preprocessed_lists = self.backend.preprocess_individual_items(
        ...
        if position_list is None:
            position_list = [
                torch.arange(ids.size(0), device=ids.device, dtype=torch.long)
                for ids in ids_list
            ]
        for item_idx, (ids, h_states, item_loss_mask, item_position_ids) in enumerate(
            ...
            if self.backend.model_type == "eagle3" and use_logits:
                ...
                if target_logprobs_item is not None:
                    target_logprobs_train_start = _eagle_target_logprobs_train_start(
                        source_item
                    )
            if self.backend.model_type == "eagle3" and use_logits:
                train_seq_len = min(
            ...
            if alignment_debug_enabled():
                ...
                if (
                    ...
                    train_target_logprobs = target_logprobs_item[
                        target_logprobs_train_start : target_logprobs_train_start
                        + train_seq_len
                    ]
                ...
                if isinstance(source_hidden_positions, torch.Tensor):
                    ...
                    base_hidden_position_start = (
                        int(train_hidden_positions[0].item())
                        if int(train_hidden_positions.numel()) > 0
                        else None
                    )
                    base_hidden_position_end = (
                        int(train_hidden_positions[-1].item()) + 1
                        if int(train_hidden_positions.numel()) > 0
                        else None
                    )
                    last_hidden_position_start = (
                        ...
                        and int(source_hidden_positions.numel()) > 1
                        else None
                    )
                    last_hidden_position_end = (
                        int(source_hidden_positions[train_seq_len].item()) + 1
                        if uses_shifted_eagle_inputs
                        and int(source_hidden_positions.numel()) > train_seq_len
                        else None
                    )
                else:
                    base_hidden_position_start = source_item.get("_verl_feature_start")
                    base_hidden_position_end = (
                        int(source_item.get("_verl_feature_start", 0) or 0)
                        + train_seq_len
                    )
                    last_hidden_position_start = (
                ...
                effective_target_position_start = source_item.get(
                    "_verl_target_position_start"
                )
                effective_target_position_end = source_item.get(
                    "_verl_target_position_end"
                )
                ...
                force_alignment_log = (
                    train_target_logprobs is not None
                    and active_valid is not None
                    and active_valid <= 0
                )
                if should_log_alignment(
                    current_step, self.rank, sample_index, force=force_alignment_log
                ):
                    log_alignment_event(
                        logger,
                        {
                            ...
                            "target_len": int(target_logprobs_item.size(0))
                            if target_logprobs_item is not None
                            else None,
                            ...
                            "hidden_position_start": source_item.get(
                                "_verl_hidden_position_start"
                            ),
                            "uses_hidden_positions": source_item.get(
                                "_verl_uses_hidden_positions"
                            ),
                            ...
                            "target_train_position_start": (
                                int(
                                    source_item.get("_verl_target_position_start", 0)
                                    or 0
                                )
                                + int(target_logprobs_train_start)
                            )
                            if source_item.get("_verl_target_position_start")
                            is not None
                            else None,
                            ...
                            "target_tensor_position_start": source_item.get(
                                "_verl_target_tensor_position_start"
                            ),
                            "target_tensor_position_end": source_item.get(
                                "_verl_target_tensor_position_end"
                            ),
                            "item_global_step": source_item.get("global_step"),
                            "hidden_lm_head_fingerprint": source_item.get(
                                "hidden_lm_head_fingerprint"
                            ),
                            "hidden_last_hidden_logprob_check": source_item.get(
                                "hidden_last_hidden_logprob_check"
                            ),
                            "hidden_target_logprobs_source": source_item.get(
                                "hidden_target_logprobs_source"
                            ),
                            "hidden_raw_topk_logprob_check": source_item.get(
                                "hidden_raw_topk_logprob_check"
                            ),
                            "hidden_raw_target_shape": _tensor_shape(
                                source_item.get("hidden_raw_target_logprobs")
                            ),
                            "hidden_raw_target_positions_shape": _tensor_shape(
                            ...
                            "hidden_last_hidden_filter": source_item.get(
                                "hidden_last_hidden_filter"
                            ),
                            "hidden_last_hidden_select": source_item.get(
                                "hidden_last_hidden_select"
                            ),
                            "target_lm_head_weight_step": getattr(
                            ...
                            "target_rows": int(train_target_logprobs.size(0))
                            if train_target_logprobs is not None
                            else None,
                            "row_valid": row_valid_count,
                            "active_rows": active_rows,
                    ...
                    window_feature_start = int(
                        source_item.get("_verl_feature_start", 0) or 0
                    )
                    if uses_shifted_eagle_inputs:
        ...
        if self._is_block_drafter_backend():
            ...
            if target_last_hidden_state_chunks:
                ...
                for row_idx, target_h_chunk in enumerate(
                    target_last_hidden_state_chunks
                ):
                    target_last_hidden_states[row_idx, : target_h_chunk.size(0)] = (
                        target_h_chunk
                    )
        else:
            ...
            position_ids = (
                torch.cat(position_id_chunks, dim=0).unsqueeze(0).contiguous()
            )
        ...
        if self.backend.model_type == "eagle3":
            if use_logits:
                if not target_logprob_chunks:
                    return None
                target_logprobs = (
                    torch.cat(target_logprob_chunks, dim=0).unsqueeze(0).contiguous()
                )
            else:
        ...
        if self.backend.model_type == "eagle3":
            ...
            batch["seq_lengths"] = torch.tensor(
                [chunk.size(0) for chunk in input_id_chunks],
                dtype=torch.long,
                device=dev,
            )
        ...
        if self.use_ulysses_sp:
            ...
            if pad_size > 0:
                ...
                if self.backend.model_type == "eagle3":
                    if use_logits:
                        ...
                        target_logprobs = torch.cat(
                            (target_logprobs, target_logprobs_pad), dim=1
                        )
                    else:
            ...
            if self.backend.model_type == "eagle3":
                if use_logits:
                    target_logprobs = slice_input_tensor(
                        target_logprobs, dim=1, padding=False
                    )
                else:
                ...
                target_last_hidden_states = slice_input_tensor(
                    target_last_hidden_states, dim=1, padding=False
                )
        ...
        if alignment_debug_enabled():
            ...
            if (
                ...
                if self.backend.model_type == "eagle3" and use_logits:
                    ...
                    if (
                        ...
                        and final_row_valid_mask.numel() > 0
                    ):
                        final_loss_mask_slice = final_loss_mask.squeeze(0)[
                            : final_row_valid_mask.size(0)
                        ].bool()
                        final_row_valid = int(
                            final_row_valid_mask.detach().sum().cpu().item()
                        )
                        final_active_rows = int(
                            final_loss_mask_slice.detach().sum().cpu().item()
                        )
                        final_active_valid = int(
                            (final_row_valid_mask & final_loss_mask_slice)
                            .detach()
                            .sum()
                            .cpu()
                            .item()
            ...
            force_alignment_log = (
                final_active_valid is not None and final_active_valid <= 0
            )
            sample_index = self._alignment_debug_sample_index(
            ...
            if should_log_alignment(
                ...
                log_alignment_event(
                    logger,
                    {
                        ...
                        "source_global_steps": [
                            item.get("global_step") for item in items[:8]
                        ],
                        ...
                        "source_last_hidden_filters": [
                            item.get("hidden_last_hidden_filter") for item in items[:2]
                        ],
                        "source_last_hidden_selects": [
                            item.get("hidden_last_hidden_select") for item in items[:2]
                        ],
                        "target_lm_head_weight_step": getattr(
                            self, "_target_lm_head_weight_step", None
                        ),
                        ...
                        "loss_sum": int(
                            final_loss_mask.detach().float().sum().cpu().item()
                        ),
                    },
                )
        ...
        try:
            for sample in samples:
                if isinstance(sample, DraftFeatureSample):
                    ...
                    item = DraftFeatureSample.from_dict(
                        sample, strict=False
                    ).to_training_item()
                else:
                    raise TypeError(
                        f"Unsupported draft feature sample type: {type(sample)!r}"
                    )
    ...
    async def _training_step_on_batch(
        ...
        reduce_ts = time.time()
        global_vloss, global_ploss, global_tokens, reduce_world_size = (
            self._reduce_loss_metrics(l_v, l_p, l_n)
        )
        self.record_training_timing(
    ...
    async def cleanup_training(self, clear_data: bool = True):
        ...
        if self._pending_full_checkpoint_future is not None:
            ...
            try:
                ...
                logger.debug(
                    f"[Rank {self.rank}] Pending full drafter checkpoint save completed"
                )
            except Exception as e:  # noqa: BLE001
    ...
    async def release_training_memory_after_activation(self):
        ...
        if self.optimizer is not None:
            try:
                ...
                logger.debug(
                    f"Failed to clear drafter gradients after activation warmup: {e}"
                )
        ...
        if self.model is not None:
            try:
                ...
                logger.debug(
                    f"Failed to offload drafter model after activation warmup: {e}"
                )
