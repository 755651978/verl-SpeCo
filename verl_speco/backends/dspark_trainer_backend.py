...
class DSparkTrainingModel(DFlashTrainingModel):
    ...
    def _sample_anchor_positions(self, seq_len: int, loss_mask: torch.Tensor, device: torch.device):
        ...
        indices = self._cached_arange("dspark_anchor_indices", num_candidates, device).unsqueeze(0).expand(bsz, -1)
        ...
        keep_mask = self._cached_arange("dspark_anchor_keep", self.num_anchors, device).unsqueeze(0) < valid_counts.unsqueeze(1).clamp(max=self.num_anchors)
    ...
    def _build_label_tensors(
        ...
        *,
        ...
        label_offsets = self._cached_arange("dspark_label_offsets", self.block_size, device, view_shape=(1, 1, -1)) + 1
        ...
        target_loss_mask = torch.gather(loss_mask.unsqueeze(1).expand(-1, n_blocks, -1), 2, safe_label_indices)
        eval_mask = valid_label_mask & (target_loss_mask > 0.5) & block_keep_mask.unsqueeze(-1)
        eval_mask = eval_mask.to(torch.int32).cumprod(dim=-1).bool()

        anchor_token_ids = torch.gather(input_ids, 1, anchor_positions.clamp(0, max(seq_len - 1, 0)))
    ...
    def _markov_bias_for_active(
        ...
        *,
        ...
        if markov_head.__class__.__name__ == "DSparkRNNMarkovHead":
            raise NotImplementedError("DSpark rnn markov_head_type requires full-block logits and is not supported yet")
        ...
        if hasattr(markov_head, "gate_proj"):
            gate = torch.sigmoid(markov_head.gate_proj(torch.cat([active_hidden, prev_embeddings], dim=-1)))
        ...
        *,
        ...
        if valid_anchor.numel() > 0:
            ...
            preview = {
                ...
                "labels": target_ids[batch_idx, block_idx].detach().cpu().tolist()[: min(self.block_size, 8)],
                "prev": prev_token_ids[batch_idx, block_idx].detach().cpu().tolist()[: min(self.block_size, 8)],
                "eval": eval_mask[batch_idx, block_idx].to(torch.int32).detach().cpu().tolist()[: min(self.block_size, 8)],
            }
        logger.info(
    ...
    def forward(
        ...
        if self.loss_decay_gamma is not None and self.loss_decay_gamma > 0:
            positions = self._cached_arange("dspark_decay_positions", self.block_size, device, view_shape=(1, 1, -1))
            weight_mask = weight_mask * torch.exp(-positions.float() / float(self.loss_decay_gamma))
        ...
        if active_targets.numel() == 0:
            ...
            if self.loss_mode in {"restricted_ce", "sampled_ce"}:
                restricted_vocab = self._build_restricted_vocab(input_ids, active_targets, int(lm_head_weight.shape[0]))
        ...
        with torch.no_grad():
            ...
            sampled_vocab_size = torch.tensor(
                float(restricted_vocab.numel()) if restricted_vocab is not None else float(lm_head_weight.shape[0]),
                dtype=torch.float32,
                device=device,
            )
            if active_logits is not None and active_targets.numel() > 0:
                if restricted_vocab is not None:
                    ...
                    top_tokens = restricted_vocab[active_logits.topk(topk, dim=-1).indices]
                else:
                ...
                top5_correct = top_tokens.eq(active_targets.unsqueeze(-1)).any(dim=-1).float().sum()
            ...
            loss_per_position = loss_sum_per_position / count_per_position.clamp(min=1.0)
            ...
            accuracy = correct.float().sum() / binary_eval_mask.float().sum().clamp(min=1.0)
...
class DSparkTrainerBackend(DFlashTrainerBackend):
    ...
    def _training_value(self, training_cfg, dspark_key: str, dflash_key: str, default: Any):
    ...
    def _normalize_dflash_config(self, drafter_config, target_hf_config, normalized_state, spec_model_path):
        ...
        if training_cfg.get("dspark_num_target_layers", None) is not None:
            if getattr(drafter_config, "num_context_layers", None) is None:
                drafter_config.num_context_layers = int(training_cfg["dspark_num_target_layers"])
    ...
    def _build_fallback_config(self, target_hf_config):
        ...
        hidden_size_cfg = self._training_value(training_cfg, "dspark_hidden_size", "dflash_hidden_size", None)
        hidden_size = int(hidden_size_cfg if hidden_size_cfg is not None else target_text_config.hidden_size)
        num_context_layers = int(self._training_value(training_cfg, "dspark_num_target_layers", "dflash_num_target_layers", 5))
        target_num_hidden_layers = int(getattr(target_text_config, "num_hidden_layers", 36))
        mask_token_id_cfg = self._training_value(training_cfg, "dspark_mask_token_id", "dflash_mask_token_id", None)
        ...
        target_layer_ids = self._training_value(training_cfg, "dspark_target_layer_ids", "dflash_target_layer_ids", None)
        ...
        return DSparkConfig(
            ...
            num_hidden_layers=int(self._training_value(training_cfg, "dspark_num_hidden_layers", "dflash_num_hidden_layers", 1)),
    ...
    def build_model(self):
        ...
        if config_path and os.path.exists(config_path):
            ...
            if spec_model_path and os.path.exists(spec_model_path):
                log_drafter_checkpoint_step(logger, spec_model_path, action="Loading DSpark drafter weights")
        ...
        if not isinstance(drafter_config, DSparkConfig):
            raise TypeError(f"DSpark config is not a DSparkConfig: {type(drafter_config)}")
        ...
        return DSparkTrainingModel(
            draft_model=draft_model,
            block_size=int(training_cfg.get("dspark_block_size", getattr(drafter_config, "block_size", 7))),
            num_anchors=int(training_cfg.get("dspark_num_anchors", getattr(drafter_config, "num_anchors", 512))),
    ...
    def preprocess_individual_items(self, items, device, model_config):
        ...
        for item in items:
            ...
            if item.get("loss_mask") is not None:
                ...
                item_loss_mask[prompt_len : prompt_len + responses.size(0)] = (responses != pad_id).float()[
                    : max(0, ids.size(0) - prompt_len)
                ]
            else:
                item_loss_mask = torch.zeros_like(ids, dtype=torch.float32)
                item_loss_mask[:] = 1.0
    ...
    def compute_loss(self, model, batch, _current_pad_size):
        if getattr(self, "use_ulysses_sp", False):
            raise NotImplementedError("DSpark drafter training does not support Ulysses sequence parallel yet")
