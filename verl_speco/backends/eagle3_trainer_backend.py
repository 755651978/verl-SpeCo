...
class Eagle3TrainerBackend:
    ...
    def compute_loss(self, model, batch, _current_pad_size):
        ...
        quality_step_stats = []
        collect_diagnostics = bool(getattr(self, "enable_standalone_training_metrics", False))
        loss_sum_per_position = None
        correct_per_position = None
        count_per_position = None
        if collect_diagnostics:
            loss_sum_per_position = torch.zeros(length, device=input_ids.device, dtype=torch.float32)
            correct_per_position = torch.zeros(length, device=input_ids.device, dtype=torch.float32)
            count_per_position = torch.zeros(length, device=input_ids.device, dtype=torch.float32)
